...
def _manual_value_filled(value: Any) -> bool:
    if value is None:
        return False
    text = str(value).strip()
    return bool(text) and not text.startswith("MANUAL_REQUIRED")
...
def _positive_int(value: Any) -> bool:
    if isinstance(value, bool):
        return False
    try:
        return int(value) == float(value) and int(value) > 0
    except (TypeError, ValueError):
        return False
...
def _actual_file_summary(path: Path) -> dict[str, Any]:
    payload = _read_optional_json(path) if path.exists() else {}
    return {
        "path": str(path),
        "present": path.exists(),
        "sha256": sha256_json(payload) if payload else None,
        "unsafe_truthy_fields": _unsafe_fields(payload),
