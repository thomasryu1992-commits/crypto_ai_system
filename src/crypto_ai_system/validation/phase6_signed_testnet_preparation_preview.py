...
def _artifact_summary(name: str, payload: Mapping[str, Any]) -> dict[str, Any]:
    data = dict(payload or {})
    return {
        "artifact_name": name,
        "present": bool(data),
        "status": data.get("status"),
        "sha256": data.get(f"{name}_sha256") or data.get("report_sha256") or data.get("signed_testnet_pre_submit_validation_sha256") or data.get("signed_testnet_execution_enablement_packet_sha256") or data.get("signed_testnet_execution_record_sha256") or sha256_json(data) if data else None,
    }
