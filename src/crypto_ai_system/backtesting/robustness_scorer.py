"""Pre-deployment overfitting risk: is this edge real, or fitted to noise?

The absolute gate asks whether a strategy's backtest looks good. It cannot ask
whether looking good *means* anything. A strategy carrying four tuned numbers and
eight trades will clear an expectancy threshold by chance often enough that
clearing it is not evidence — and nothing downstream distinguishes that from a
strategy that earned the same number over hundreds of trades.

This module scores that separately, from evidence the backtest already produces:
how many observations back each fitted parameter, whether the edge survives
across time slices and across regimes, and how much of it the costs eat. The
dominant term is deliberately observations-per-parameter, because that is the one
that decides whether the other terms are measuring an edge or measuring noise.

Scoring only. It ranks and warns; whether a verdict blocks a strategy is the
gate's decision (``AbsoluteGate.min_trades_per_parameter`` /
``min_robustness_score``), and off by default so it reports before it enforces.

Pure: no IO, no runtime state, no order path.
"""
...
# Exit rules always carry three fitted numbers: stop_atr, target_atr,
# max_holding_bars. Every generated spec pays this cost.
EXIT_FREE_PARAMETERS = 3
# The validator caps entry conditions at 8, so 11 is the worst case and 3 (all
# structural entry comparisons + the exits) is the floor.
MAX_FREE_PARAMETERS = 11
MIN_FREE_PARAMETERS = EXIT_FREE_PARAMETERS

# Rule of thumb: an estimate needs ~10 observations per fitted parameter to mean
# anything; under ~5 it is indistinguishable from noise regardless of how good it
# looks.
...
def count_free_parameters(spec: StrategySpec) -> int:
    """How many numbers in this strategy could have been fitted.

    A condition comparing two features (``value_from``, e.g. ``close > ma20``)
    states a structural relationship and has nothing to tune. A literal — a
    numeric threshold or a categorical label — is a value someone chose and could
    have chosen differently, so each one is a degree of freedom.
    """
...
def _temporal_consistency(walk_forward: Mapping[str, Any]) -> float:
    """Credit only what the walk-forward actually demonstrated.

    Both signals are None when too few windows traded to compare them. That is an
    absence of evidence, not evidence of stability, so it scores zero rather than
    being skipped."""
    parts = [
        v
...
def _regime_breadth(regime_breakdown: Mapping[str, Any]) -> float:
    """An edge that only appears in one of several regimes is fitted to that
    regime, not to the market."""
...
def _cost_robustness(metrics: Mapping[str, Any]) -> float:
    """What fraction of the pre-cost edge survives fees and slippage.

    ``total_net_r`` is already net, so the gross edge is it plus the costs. An
    edge that only exists before costs is not an edge."""
...
def _verdict(score: float, trades_per_parameter: float) -> str:
    # The veto is not a tiebreak. Below the critical ratio every other component
    # is computed on the same too-small sample, so a high score there is not
    # reassurance — it is the overfitting showing up as confidence.
    if trades_per_parameter < CRITICAL_TRADES_PER_PARAMETER:
        return FRAGILE
...
def score_robustness(
    ...
    record["robustness_id"] = stable_id("robustness", record, 24)
    return record
