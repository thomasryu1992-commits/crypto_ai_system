"""The declarative strategy spec — Phase S1's central contract.

A :class:`StrategySpec` is data, not code: entry/exit rules expressed as typed
conditions over named features. ``from_dict`` does *structural* validation only
(required fields, types, enum membership, well-formed conditions) and raises
:class:`SpecParseError` on malformed input. It intentionally does **not** decide
whether a strategy is *safe or good* — feature-existence, look-ahead, parameter
ranges and stop-loss policy are the Strategy Validation Agent's job (Phase S3).
Keeping structure here and safety there mirrors the directive's S1/S3 split.

Hard safety invariant: a spec can never carry execution authority.
``can_submit_orders`` and ``can_modify_runtime`` are forced false, and a dict
that tries to set either true is rejected.
"""
...
# Timeframes the backtest/runtime feature pipeline supports.
ALLOWED_TIMEFRAMES = frozenset({"15m", "1h", "4h", "1d"})

# Exit stop models understood by the exit layer. ATR-based is the one wired.
...
class StrategyStatus(str, Enum):
    """Lifecycle status. S1 only produces GENERATED; later phases advance it."""
...
class RuleCondition:
    """One entry condition: ``feature comparison (value | value_from)``.

    Exactly one of ``value`` (a constant — numeric for numeric features, a label
    string for categorical ones) or ``value_from`` (another feature name) is set:
    a condition compares a feature either to a constant or to another feature,
    never both and never neither. Whether a string value is a *valid* label for a
    given feature is the S3 validator's call, not this structural parse.
    """
    ...
    def from_dict(raw: Any, *, where: str) -> "RuleCondition":
        ...
        if has_value:
            ...
            if isinstance(raw_value, (int, float)):
                ...
                raise SpecParseError(f"{where}: condition 'value' must be a number or non-empty string, got {raw_value!r}")
    ...
    def from_dict(raw: Any) -> "ExitRules":
        ...
        def _pos_number(key: str) -> float:
            ...
            try:
                ...
                raise SpecParseError(f"exit_rules.{key} must be numeric, got {val!r}")
            if num <= 0:
        ...
        try:
            ...
            raise SpecParseError(f"exit_rules.max_holding_bars must be an integer, got {max_holding_raw!r}")
...
class RiskConstraints:
    ...
    def from_dict(raw: Any) -> "RiskConstraints":
        ...
        try:
            ...
            raise SpecParseError(f"risk_constraints.max_risk_per_trade_R must be numeric, got {val!r}")
        if num <= 0:
...
class StrategySpec:
    ...
    # Hard safety: a spec never carries execution authority.
    can_submit_orders: bool = field(default=False, init=False)
    can_modify_runtime: bool = field(default=False, init=False)
    ...
    def from_dict(raw: Any) -> "StrategySpec":
        ...
        # Reject any attempt to grant execution authority (fail-closed).
        ...
        return spec.with_rule_hash(computed)

    def with_rule_hash(self, rule_hash: str) -> "StrategySpec":
        return _replace_frozen(self, strategy_rule_hash=rule_hash)

    def with_status(self, status: StrategyStatus) -> "StrategySpec":
        """Return a copy at a new lifecycle status. Rule hash is unchanged —
        status is metadata, not part of a strategy's identity."""
        return _replace_frozen(self, status=status)

    def referenced_features(self) -> set[str]:
        """All feature names the entry rules touch (for the S3 validator)."""
        names: set[str] = set()
...
def _parse_enum(value: Any, enum_cls: type[Enum], key: str) -> Any:
    ...
    try:
        ...
        raise SpecParseError(f"{key} must be one of {allowed}, got {value!r}")


def _replace_frozen(spec: StrategySpec, **changes: Any) -> StrategySpec:
    # dataclasses.replace can't be used directly because can_submit_orders /
    # can_modify_runtime are init=False; rebuild through the normal fields.
    data = {
        ...
        "symbol_scope": spec.symbol_scope,
