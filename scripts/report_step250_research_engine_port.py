...
def build_step250_batch7_report(root: Path) -> dict:
    ...
    ported_remaining = sorted(set(BATCH7_PORTED_LEGACY_MODULES) & remaining_legacy_modules)

    return {
        "report_type": "step250_research_engine_canonical_port_report",
        "status": "BATCH7_CANONICAL_PORT_APPLIED_RESEARCH_ONLY",
        "batch_scope": "RESEARCH_ENGINE_DECISION_CANONICAL_PORT",
        "ported_modules": BATCH7_PORTED_LEGACY_MODULES,
        "support_modules_ported": BATCH7_SUPPORT_MODULES,
        ...
        "next_step": {
            "name": "Step251 v5 Trading Cycle Canonical Port Batch",
            "goal": "Port the final trading.trading_cycle group with tests and limited import rewrite.",
        },
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Build Step250 research engine canonical port report.")
    parser.add_argument("--root", default=".", help="Repository root")
    parser.add_argument("--output", default="data/reports/step250_research_engine_canonical_port_report.json")
    ...
    report = build_step250_batch7_report(root)
    ...
    print(json.dumps({
        ...
        "research_engine_mode": report["research_engine_mode"],
        "research_decision_mode": report["research_decision_mode"],
