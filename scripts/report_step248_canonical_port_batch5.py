...
BATCH5_PORTED_LEGACY_MODULES = {
    "execution.order_executor": "crypto_ai_system.execution.order_executor",
}


def build_step248_batch5_report(root: Path) -> dict:
    ...
    ported_remaining = sorted(set(BATCH5_PORTED_LEGACY_MODULES) & remaining_legacy_modules)

    return {
        "report_type": "step248_canonical_port_batch5_report",
        "status": "BATCH5_CANONICAL_PORT_APPLIED_GUARDED_REVIEW_ONLY",
        "batch_scope": "EXECUTION_ORDER_EXECUTOR_GUARDED_CANONICAL_PORT",
        "ported_modules": BATCH5_PORTED_LEGACY_MODULES,
        ...
        "next_step": {
            "name": "Step249 v5 Execution Reconciler Canonical Port Batch",
            "goal": "Port execution.reconciler and/or execution.execution_reconciler with tests and limited import rewrite.",
        },
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Build Step248 canonical port batch 5 report.")
    parser.add_argument("--root", default=".", help="Repository root")
    parser.add_argument("--output", default="data/reports/step248_canonical_port_batch5_report.json")
    ...
    report = build_step248_batch5_report(root)
    ...
    print(json.dumps({
        ...
        "order_executor_mode": report["order_executor_mode"],
        "live_trading_allowed": report["live_trading_allowed"],
        "adapter_routing_enabled": report["adapter_routing_enabled"],
