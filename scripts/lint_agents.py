...
def build_agent_lint_report(root: str | Path = ".") -> dict[str, Any]:
    ...
    report = {
        "agent_lint_report_version": "step321_agent_lint_v1",
        "created_at_utc": utc_now_canonical(),
        "status": "AGENT_LINT_PASSED" if not errors else "AGENT_LINT_BLOCKED",
        "passed": not errors,
        "review_only": True,
        ...
        "agent_ids": [record["agent_id"] for record in contract_records],
        "errors": errors,
        "contracts": contract_records,
    }
    report["agent_lint_report_sha256"] = sha256_json(report)
    return report


def persist_agent_lint_report(report: dict[str, Any], root: str | Path = ".") -> Path:
    latest = Path(root).resolve() / "storage" / "latest"
    latest.mkdir(parents=True, exist_ok=True)
    target = latest / "agent_lint_report.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_agent_lint_report(root)
    path = persist_agent_lint_report(report, root)
