...
def main() -> int:
    ...
    if result.status != STEP236_STATUS_OK or validation.status != STEP236_VALIDATION_OK:
        print("STEP236_V5_ENABLEMENT_SUBMIT_GATE_REVIEW_ONLY_FAILED")
        print(validation.to_dict())
        return 1
    print("STEP236_V5_ENABLEMENT_SUBMIT_GATE_REVIEW_ONLY_OK")
    ...
    print(f"submit_gate_record_count: {result.submit_gate_record_count}")
    print(f"submit_gate_review_ready_count: {result.submit_gate_review_ready_count}")
    print(f"submit_gate_blocked_count: {result.submit_gate_blocked_count}")
    print(f"enablement_submit_gate_review_created: {result.enablement_submit_gate_review_created}")
    print(f"submit_gate_mode: {result.submit_gate_mode}")
    print(f"submit_gate_review_only: {result.submit_gate_review_only}")
