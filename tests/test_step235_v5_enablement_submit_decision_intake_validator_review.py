...
def test_step235_creates_submit_decision_intake_records_review_only(isolated_project_root):
    root = isolated_project_root
    result = execute_enablement_submit_decision_intake_validator_review(root, write_output=True)
    assert result.status == STEP235_STATUS_OK
    assert result.submit_decision_intake_validator_created is True
    assert result.intake_validator_mode == INTAKE_VALIDATOR_MODE
    assert result.intake_validator_review_only is True
    assert result.source_submit_decision_count > 0
    assert result.intake_record_count > 0


def test_step235_missing_input_defaults_not_approved_and_stays_disabled(isolated_project_root):
    root = isolated_project_root
    input_path = root / "config/enablement_submit_decision_input.json"
    if input_path.exists():
        input_path.unlink()
    result = execute_enablement_submit_decision_intake_validator_review(root, write_output=True)
    assert result.submit_decision_input_present is False
    ...
    for row in result.intake_records:
        assert row["decision_status"] == DEFAULT_STATUS
        assert row["submit_decision_accepted"] is False
...
def test_step235_validation_artifacts_are_valid(isolated_project_root):
    root = isolated_project_root
    ...
    assert validation.status == STEP235_VALIDATION_OK
    assert validation.result_hash_valid is True
    assert validation.source_step234_present is True
    ...
    assert validation.source_submit_decisions_present is True
