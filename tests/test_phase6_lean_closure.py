...
def test_phase6_closure_manifest_is_review_only_and_fail_closed() -> None:
...
def test_readiness_and_execution_permission_states_remain_all_false() -> None:
    readiness_state = (
        readiness_review_permission_state()
    )
...
def test_lean_migration_state_advances_to_phase8_without_permission_change() -> None:
    root = Path(__file__).resolve().parents[1]
    ...
    assert (
        payload["execution_permissions_changed"]
        is False
    )


def test_phase6_history_is_consolidated_without_granting_permission() -> None:
    root = Path(__file__).resolve().parents[1]
...
def test_phase6_historical_build_commands_do_not_use_legacy_validation_imports() -> None:
    root = Path(__file__).resolve().parents[1]

    for path in (
        root / "scripts"
    ).glob("build_phase6*.py"):
