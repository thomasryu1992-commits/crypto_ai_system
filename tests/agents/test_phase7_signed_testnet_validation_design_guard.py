...
from crypto_ai_system.validation.phase7_signed_testnet_validation_design_guard import (
...
def _prepare_sources_without_actual_files() -> None:
...
def _create_actual_files() -> None:
    ...
    approval.update(
        {
            "approval_packet_id": "approval_packet_manual_phase7_test_001",
            "approval_intake_id": "approval_intake_manual_phase7_test_001",
            "approver_info": "Thomas",
            "ticket_or_signature": "MANUAL_APPROVAL_PHASE7_TEST_SIGNATURE",
            "canonical_utc_timestamp": "2026-07-02T04:45:00Z",
        }
    )
    unlock.update(
        {
            "operator_id": "Thomas",
            "operator_ticket_or_signature": "OPERATOR_UNLOCK_PHASE7_TEST_SIGNATURE",
            "canonical_utc_timestamp": "2026-07-02T04:46:00Z",
            "approval_intake_id": "approval_intake_manual_phase7_test_001",
            "approval_packet_id": "approval_packet_manual_phase7_test_001",
...
def test_phase7_design_guard_blocks_without_phase6_6_ready_bridge() -> None:
    _prepare_sources_without_actual_files()
    report = build_phase7_signed_testnet_validation_design_guard_report()

    assert report["status"] == STATUS_BLOCKED_REVIEW_ONLY
    assert report["blocked"] is True
    assert report["fail_closed"] is True
    assert "PHASE6_6_BRIDGE_NOT_READY_FOR_PHASE7_DESIGN_REVIEW" in report["block_reasons"]
    assert report["phase7_design_ready_review_only"] is False
...
def test_phase7_design_guard_records_design_packet_but_keeps_executor_disabled() -> None:
    _prepare_sources_without_actual_files()
    ...
    assert report["phase7_design_ready_review_only"] is True
    assert report["phase7_validation_design_packet_created"] is True
    assert report["disabled_executor_guard_report_created"] is True
    assert report["phase7_execution_authority"] is False
    assert report["phase7_order_submission_authority"] is False
    assert report["ready_for_signed_testnet_execution"] is False
    assert report["testnet_order_submission_allowed"] is False
    ...
    assert report["signed_order_executor_enabled"] is False
    assert Path("storage/latest/phase7_signed_testnet_validation_design_guard_report.json").exists()
    assert Path("storage/latest/signed_testnet_validation_design_packet_review_only.json").exists()
    assert Path("storage/latest/signed_testnet_disabled_executor_guard_report.json").exists()
    assert Path("storage/latest/SIGNED_TESTNET_VALIDATION_DESIGN_HANDOFF_REVIEW_ONLY.md").exists()
    assert Path("storage/registries/phase7_signed_testnet_validation_design_guard_registry.jsonl").exists()
