...
def main() -> int:
    parser = argparse.ArgumentParser(description="External GET-only Extended P71 account probe using Windows Credential Manager")
    ...
    print(json.dumps({"receipt_sha256": receipt.get("receipt_sha256"), "actual_network_read_performed": receipt.get("actual_network_read_performed"), "write_call_performed": False}, sort_keys=True))
    return 0 if receipt.get("actual_network_read_performed") else 1
