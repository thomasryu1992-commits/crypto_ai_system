
from scripts.common import bootstrap

bootstrap()

from pathlib import Path

from crypto_ai_system.ops.paper_execution_enablement_request_final_validator_review import (
    STEP229_STATUS_OK,
    STEP229_VALIDATION_OK,
    execute_paper_execution_enablement_request_final_validator_review,
    validate_paper_execution_enablement_request_final_validator_review,
)


def main() -> int:
    root = Path(__file__).resolve().parent
    result = execute_paper_execution_enablement_request_final_validator_review(root, write_output=True)
    validation = validate_paper_execution_enablement_request_final_validator_review(root)
    if result.status != STEP229_STATUS_OK or validation.status != STEP229_VALIDATION_OK:
        print("STEP229_V5_PAPER_EXECUTION_ENABLEMENT_REQUEST_FINAL_VALIDATOR_REVIEW_ONLY_FAILED")
        print(validation.to_dict())
        return 1
    print("STEP229_V5_PAPER_EXECUTION_ENABLEMENT_REQUEST_FINAL_VALIDATOR_REVIEW_ONLY_OK")
    print(f"source_request_stub_count: {result.source_request_stub_count}")
    print(f"source_request_stub_review_ready_count: {result.source_request_stub_review_ready_count}")
    print(f"final_validation_record_count: {result.final_validation_record_count}")
    print(f"final_validation_review_ready_count: {result.final_validation_review_ready_count}")
    print(f"final_validation_blocked_count: {result.final_validation_blocked_count}")
    print(f"final_validator_review_created: {result.final_validator_review_created}")
    print(f"final_validator_mode: {result.final_validator_mode}")
    print(f"final_validator_review_only: {result.final_validator_review_only}")
    print(f"final_enablement_request_validation_passed: {result.final_enablement_request_validation_passed}")
    print(f"enablement_request_submit_allowed: {result.enablement_request_submit_allowed}")
    print(f"enablement_request_submitted: {result.enablement_request_submitted}")
    print(f"paper_execution_enablement_allowed: {result.paper_execution_enablement_allowed}")
    print(f"paper_execution_enabled: {result.paper_execution_enabled}")
    print(f"paper_order_execution_enabled: {result.paper_order_execution_enabled}")
    print(f"paper_trade_execution_enabled: {result.paper_trade_execution_enabled}")
    print(f"adapter_routing_enabled: {result.adapter_routing_enabled}")
    print(f"shadow_execution_enabled: {result.shadow_execution_enabled}")
    print(f"live_trading_allowed: {result.live_trading_allowed}")
    print(f"strategy_registry_write_allowed: {result.strategy_registry_write_allowed}")
    print(f"live_order_executed: {result.live_order_executed}")
    print(f"telegram_real_send: {result.telegram_real_send}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
